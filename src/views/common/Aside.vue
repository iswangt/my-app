<template>
  <el-aside class="aside">
    <el-menu :default-active="defaultAct"
             router
             :collapse="collapse">
      <span v-for="(item,index) in menuItem"
            :key="index">
        <!-- 有子路由目录 -->
        <el-submenu v-if="!item.meta.hidden &&item.meta.child">
          ...
          <el-menu-item-group v-for="(ele,idx) in item.children"
                              :key="idx">
        ...
        <el-menu-item :key="index"
                      :index="item.path"
                      v-if="!item.meta.hidden && !item.meta.child">
          <i :class="item.meta.icon"></i>
          <span slot="title">{{$t(`message.bread.${item.meta.localekey}`)}}</span>
        </el-menu-item>
      </span>
